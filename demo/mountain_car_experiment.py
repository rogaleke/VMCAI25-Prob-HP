...
import sys
sys.path.append(".")
from demo import performance_test
...
def run_profile_mcar(file, max_depth=9, predicate_pos_count=10, predicate_vel_count=10, split_method='rand_split_single', split_ratio = 0.8, skip_concrete=False, timeout=180):
    ...
    print(f"LEN EFFECT STATES: {len(effects)}")

    #sanitize empty predicates:
    initial_predicates = {k:v for k,v in initial_predicates.items() if v}

    dtmc_time, abs_time, rf_steps = performance_test.performance_test(file, initial_predicates, 0, len(result)-1, refinement_method=split_method, split_ratio=split_ratio, debug=False, skip_concrete=skip_concrete, timeout=timeout)
...
if __name__ == "__main__":
    # Example usage:
    dtmc_path = r'environments\mountain_car\dtmcs\vel0.03pos0.1_time10.csv'
    run_profile_mcar(dtmc_path, max_depth=10, predicate_pos_count=100, predicate_vel_count=10, split_method='rand_split_single', split_ratio=0.61, skip_concrete=False, timeout=180)