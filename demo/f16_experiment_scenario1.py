...
def run_profile_f16_scen1(file, split_method='rand_split_single', split_ratio = 0.61, skip_concrete=False, state_col='state'):
    ...
    initial_predicate = f'{state_col} == 0'
    initial_state_predicates = df.query(initial_predicate)[state_col].tolist()
    effect_pred = f'V6 == True'
    effects = df.query(effect_pred)[state_col].tolist()

    exclusion = f'not ({effect_pred}) and not ({initial_predicate})'
    cols = ['V1', 'V2', 'V3', 'V4']
    groups = [5, 2, 5, 2]
    result = get_initial_predicates(df, cols, groups, state_col=state_col, method='cut', exclude=exclusion)
    ...
    dtmc_time, abs_time, rf_steps = performance_test.performance_test(file, initial_predicates, 0, len(result)-1, refinement_method=split_method, split_ratio=split_ratio, debug=False, skip_concrete=skip_concrete, state_col=state_col)
    if dtmc_time and abs_time:
...
if __name__ == "__main__":
    # Example usage:
    dtmc_path = r'environments\f16\dtmcs_scen1\vel1000_ex150_time10.csv'
    run_profile_f16_scen1(dtmc_path, split_method='rand_split_single', split_ratio=0.61, skip_concrete=False, state_col='state')